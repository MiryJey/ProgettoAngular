(function() {
    ...
    function directive() {
        return {
            ...
            template: '' +
                '<form>'+
                    '<input class="search-input" type="text" ng-model="search.title" name="search" placeholder="Search Task..">'+
                '</form>'+
                '            <md-content class="md-padding scroll tab-content">' +
                '    <md-list>' +
                '        <md-list-item class="md-2-line" ng-repeat="item in customListCtrl.items | filter:search:strict | orderBy: \'title\'" ng-class="customListCtrl.selectedItem == item ? \'selected\':\'\'" ng-click="customListCtrl.toggleSelection(item)">'
